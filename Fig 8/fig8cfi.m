...
% oscil
scale = @(p, alpha, beta) 1 + (1/nchoosek(size(p, 2), 3)) .* alpha .* sum( cos( beta .*  2.*pi .* (max((reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3])), [], 3) - min((reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3])), [], 3)) ) .* prod( reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3]), 3), 2);
...
export_fig(append('./figs/fig8i'), '-pdf', '-eps', '-q101');
savefig([pwd '/figs/fig8i'])

%%

i=1;
%decreasing
combos_i = [17, 44, 19, 47, 45, 3, 47, 7];
wces = [1, 1, 2, 2, 2, 3, 3, 3];
figure
for combos = combos_i
    scatter(rt_b_avg{combos,wces(i)}(2:end), bound_avg_logd{combos,wces(i)}(2:end), 10, C(wces(i),:), 'filled')
    hold on
...
export_fig(append('./figs/fig8f'), '-pdf', '-eps', '-q101');
savefig([pwd '/figs/fig8f'])

%%

i=1;
%increasing
combos_i = [9, 29, 49, 32, 43, 39, 41, 22];
wces = [1, 1, 2, 2, 2, 3, 3, 3];
figure
for combos = combos_i
    scatter(rt_b_avg{combos,wces(i)}(2:end), bound_avg_logd{combos,wces(i)}(2:end), 10, C(wces(i),:), 'filled')
    ...
    ypred = resubPredict(gprMdl_mn);
    X = rt_b_avg{combos,wces(i)}(~isnan(real(bound_avg_logd{combos,wces(i)})));
    p{i} = plot(X(2:end), ypred, 'Color', C(wces(i),:), 'LineWidth', 2);
    xlim([0 30])
    ylim([-0.5 6.5])
    hold on
    rt_i=[rt_i; rt_i];
...
set(hf, 'color', 'w', 'units', 'inches', 'position', [0 0 4 4])
set(ha, 'color', 'w', 'units', 'inches', 'position', [1 1 2.75 2.5])
title(ha, 'oscil(\theta, \alpha, \beta)', 'FontSize', 20);
ha.XAxis.TickValues = [0 10 20 30];
ha.YAxis.TickValues = [0 2 4 6];

l=legend([p{1}, p{3}, p{6}], {'{\it c/W} = 0.001', '{\it c/W} = 0.04', '{\it c/W} = 0.1'});

if ~exist('figs', 'dir')
       mkdir('figs')
    end

export_fig(append('./figs/fig8c'), '-pdf', '-eps', '-q101');
savefig([pwd '/figs/fig8c'])