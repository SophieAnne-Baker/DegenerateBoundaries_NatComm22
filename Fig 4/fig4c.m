...
figure
hold on
wcses = permute(repmat(reward.wcs, 1, 50, 50, 50), [2, 3, 4, 1]);
scatter(reshape(err_p, 1, []), reshape(rT_p, 1, []),15, reshape(wcses, 1, []),'filled')
plot(err_av, rT_av, 'k--', 'LineWidth', 1)
xlim([0, 0.5])
ylim([0, 18])
xlabel('mean error', 'fontsize', 14)
ylabel(' ', 'fontsize', 14)
title('      power(\theta, \alpha, \beta)    ', 'fontsize', 14)
colorbar('TickLabels', num2str(exp(opt.rx(1,1):diff(opt.rx(1,:))/10:opt.rx(1,2))',3));
c = colorbar;
c.Label.String = 'Cost ratio, \it{c/W}';
c.Label.FontSize = 10;
