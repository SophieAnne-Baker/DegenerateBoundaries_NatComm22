...
% scaling function
beta = 2;
scale = @(p, alpha, theta) 1 + (1/nchoosek(size(p, 2), 3)) .* alpha .* sum( (max((reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3])), [], 3) - min((reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3])), [], 3)).^(beta) .* prod( reshape(p(:, nchoosek(1:size(p, 2), 3)), [size(p, 1), nchoosek(size(p, 2), 3), 3]), 3), 2);
...
axis off
text(vertices2D(1,1), 0.66*vertices2D(2,2), ['\alpha=' num2str(alpha)],'fontsize',10)
text(vertices2D(1,1), 0.9*vertices2D(2,1), ['\beta=' int2str(beta)],'fontsize',10)
